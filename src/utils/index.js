 ...
 * @param encode 是否加密存储
 */
export function setSStorage(key, value, encode) {
...
export function getSStorage(key, decode) {
  var str = "";
...
export function setLStorage(key, value, encode) {
  let str = window.JSON.stringify(value);
  if (encode) {
    str = Crypto.encrypt(str);
  }
...
export function getLStorage(key, decode) {
  var str = "";
...
export function checkMobile(nub) {
 ...
 */
export function getFormatDate(formatType, formatDate) {
 ...
 */
export function easeout(dom, destination = 0, rate = 3) {
 ...
 */
export function formatRank(arr) {
  let count = arr.map(v => {
